...
Answer: There are 20000 cases, 9 varaibles,data types are
genhlth - categorical
exerany - categorical
exernany - categorical
...
summary(cdc$age)
# Relative frequency
round(prop.table(table(cdc$gender)),2)
round(prop.table(table(cdc$exerany)), 2)
# no of males
table(cdc$gender)
```
Thre are 9569 males in the sample.
0.75 percent are in excellent health.
...
Answer: There are more males who smokes than the number of females who smoke.
...
```{r}
ggplot(data = cdc, aes(weight, wtdesire))+geom_point()+theme_classic()+ggtitle("Scatterpot of weight versus desired weight")+xlab("Weight")+ylab("Desired Weight")
```
The relationship between these two variables is positive as with the increase of one variable another ones increase and vice verse.
...
table(cdc$wdiff)
```
wdiff is a numeric wdiff positive means the person is overweight and negative means the person is underweight.
...
skewness(cdc$wdiff)
kurtosis(cdc$wdiff)
ggplot(data = cdc, aes(cdc$wdiff))+geom_histogram()+ggtitle("Histogram of wdiff")+theme_classic()+theme(plot.title = element_text(hjust = 0.5))
```
Average difference between weight is  -14.59 which means most of the persons are overweight. A standard deviaiton os 24.04586 is small which means the difference in weights are tightly closed together.From the histogram we can see that the distribution platykurtic and approximately normally distributed.
...
ggplot(cdc, aes(x = gender, y = weight))+geom_boxplot()+theme_classic()+ggtitle("Boxplot of weight ")
```
From the numerical summary and boxplot we can see that men tends to see their weight differently then woman.
...
lo = mean - 1/2*sd
hi = mean + 1/2*sd
percent = sum(cdc$weight>=lo & cdc$weight<=hi)/length(cdc$weight) *100
mean
sd
percent
```
Mean of weight is 169.683, sd is  40.08097,39.335 % weights are within one standard deviation of the mean.
<div id="license">
